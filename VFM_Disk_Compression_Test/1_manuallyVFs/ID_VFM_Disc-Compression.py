...
class VirtualFieldsMethod:
    ...
    def compute_integrals_general(self, vf_strains_list):
        ...
        for i, vf_strains in enumerate(vf_strains_list):
            # For each virtual field, compute:
            # A[i,0] = integral(e2 * e2_star + e1 * e1_star) - coefficient for Q11
            # A[i,1] = integral(e1 * e2_star + e2 * e1_star) - coefficient for Q12
    ...
    def compute_external_work_set1(self, k1, k2):
        ...
        """
        # VF1: Virtual displacement at point A: u2* = -k1*d
        B1 = self.F * k1 * self.d / self.t

        # VF2: Virtual displacement at point A: u1* = k2*0 = 0
        B2 = 0.0
    ...
    def compute_external_work_set2(self):
        ...
        """
        # VF3: Virtual displacement at top (x2=d): u2* = -sin(π*d/(2d)) = -sin(π/2) = -1
        B3 = -self.F * (-1) / self.t  # Note: -sin(π/2) = -1, so F*(-1)/t

        # VF4: Virtual displacement at point A (x1=0): u1* = exp(0) = 1 (but A is at top, so zero horizontal force)
        B4 = 0.0
    ...
    def compute_external_work_set3(self):
        ...
        """
        # VF5: Virtual displacement at top (x2=d): u2* = -d^3
        B5 = -self.F * (-self.d ** 3) / self.t

        # VF6: Virtual displacement at point A (x1=0): u1* = exp(0) = 1 (but no horizontal force)
        B6 = 0.0
    ...
    def convert_to_engineering_constants(self, Q11, Q12, verbose=False):
        ...
        For plane stress isotropic material:
        Q11 = E/(1-ν²)
        Q12 = νE/(1-ν²)
        """
        ...
        nu = Q12 / Q11  # Poisson's ratio
        E = Q11 * (1 - nu ** 2)  # Young's modulus
        ...
        if verbose:
            ...
            # Verification
            print(f"\nVerification:")
            print(f"  Q11 from E,ν = {E / (1 - nu ** 2):.2f} MPa (should match {Q11:.2f})")
            print(f"  Q12 from E,ν = {nu * E / (1 - nu ** 2):.2f} MPa (should match {Q12:.2f})")
    ...
    def run_analysis(self, E_ref=None, nu_ref=None):
        ...
        Q11_s1, Q12_s1 = self.solve_system(A1, B1)
        if Q11_s1:
            E_s1, nu_s1 = self.convert_to_engineering_constants(Q11_s1, Q12_s1, verbose=True)
            results['Set1'] = {'E': E_s1, 'nu': nu_s1, 'Q11': Q11_s1, 'Q12': Q12_s1}
        ...
        Q11_s2, Q12_s2 = self.solve_system(A2, B2)
        if Q11_s2:
            E_s2, nu_s2 = self.convert_to_engineering_constants(Q11_s2, Q12_s2, verbose=True)
            results['Set2'] = {'E': E_s2, 'nu': nu_s2, 'Q11': Q11_s2, 'Q12': Q12_s2}
        ...
        Q11_s3, Q12_s3 = self.solve_system(A3, B3)
        if Q11_s3:
            E_s3, nu_s3 = self.convert_to_engineering_constants(Q11_s3, Q12_s3, verbose=True)
            results['Set3'] = {'E': E_s3, 'nu': nu_s3, 'Q11': Q11_s3, 'Q12': Q12_s3}
        ...
        Q11_c, Q12_c = self.solve_system(A_all, B_all, method='least_squares')
        if Q11_c:
            E_c, nu_c = self.convert_to_engineering_constants(Q11_c, Q12_c, verbose=True)
            results['Combined'] = {'E': E_c, 'nu': nu_c, 'Q11': Q11_c, 'Q12': Q12_c}
        ...
        if E_ref is not None and nu_ref is not None:
            ...
            print(f"{'Method':<20} {'E (GPa)':<25} {'ν':<25} {'Q11 (MPa)':<12} {'Q12 (MPa)':<12}")
            print("-" * 100)
            for key, v in results.items():
                ...
                print(f"{key:<20} {E_str:<25} {nu_str:<25} {v['Q11']:<12.2f} {v['Q12']:<12.2f}")
        else:
            print(f"{'Method':<20} {'E (GPa)':<12} {'ν':<12} {'Q11 (MPa)':<12} {'Q12 (MPa)':<12}")
            print("-" * 80)
            for key, v in results.items():
                E_GPa = v['E'] / 1000.0
                print(f"{key:<20} {E_GPa:<12.4f} {v['nu']:<12.6f} {v['Q11']:<12.2f} {v['Q12']:<12.2f}")

        return results

    def plot_strain_fields(self, save_figure=True, filename='strain_fields.png', dpi=300):
        """Plot strain fields"""
        if self.data is None:
            return
        ...
        ax1.set_title(r'Strain $\varepsilon_{xx}$', fontsize=FS+8)
        ax1.set_xlabel(r'$x$ (mm)', fontsize=FS)
        ax1.set_ylabel(r'$y$ (mm)', fontsize=FS)
        ax1.tick_params(labelsize=FS)
        ax1.set_aspect('equal')
        cbar1 = plt.colorbar(c1, ax=ax1, fraction=0.046, pad=0.04) # format='%.2E'
        #cbar1.set_label(r'$\varepsilon_{xx}$', fontsize=22)
        ...
        ax2.set_title(r'Strain $\varepsilon_{yy}$', fontsize=FS+8)
        ...
        #cbar2.set_label(r'$\varepsilon_{yy}$', fontsize=22)
...
if __name__ == "__main__":
    ...
    # Run analysis
    try:
        # Plot strain fields with custom settings
        vfm.plot_strain_fields(save_figure=True, filename='disc_strain_fields.png', dpi=150)

        # Run complete analysis with all sets
        # results = vfm.run_analysis()
        results = vfm.run_analysis(E_ref=E_ref, nu_ref=nu_ref)
        ...
        traceback.print_exc()
        print("\nPlease check:")
        print("1. File format (should be: Area_mm2, X_Coord, Y_Coord, U_X, U_Y, Strain_X, Strain_Y, Strain_XY)")
        print("2. File path")
        print("3. Data units (strains should be dimensionless)")